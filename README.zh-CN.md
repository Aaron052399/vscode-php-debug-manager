...
### 🔍 智能扫描
- 自动扫描项目中的所有 PHP 文件，基于 AST 解析（php-parser）精确检测
- 识别多种调试语句：`var_dump`、`print_r`、`error_log`、`debug_backtrace` 等
  - 树状层级展示：文件路径 → 行号 → 调试内容
- 精确识别调试语句，避免字符串和注释内的误报
...
### 核心组件
- `PhpAstParser`: PHP AST 解析器，基于 php-parser 库
- `DebugScanner`: 调试语句扫描器
- `DebugDataProvider`: 数据提供器和树状结构管理
- `DebugManagerView`: 调试管理器视图

### 扫描算法与守卫
- **AST 解析扫描**：使用 php-parser 进行精确的 PHP 语法分析
